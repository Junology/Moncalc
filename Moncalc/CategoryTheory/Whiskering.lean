...
/-!
The file contains definitions and theorems based on those from `Mathbin/CategoryTheory/Whiskering.lean` file in the repository of [mathlib3port](https://github.com/leanprover-community/mathlib3port) project.
The original copyright notice following:
...
def whiskeringLeft :
    (C ⥤ D) ⥤
      (D ⥤ E) ⥤
        C ⥤
          E where 
  obj F := { obj := λ G => F ⋙ G, map := λ α => whiskerLeft F α }
  map τ :=
    { app := fun H =>
...
def whiskeringRight :
    (D ⥤ E) ⥤
      (C ⥤ D) ⥤
        C ⥤
          E where 
  obj H := { obj := fun F => F ⋙ H, map := fun α => whiskerRight α H }
  map τ :=
    { app := fun F =>
...
/-
-- This instance not yet available as `Mathlib.CategoryTheory.Functor.Fullyfaithful` is in WIP.
